...
def period_div(path, murmur, patient_id_list, positoin, id_data,
               Murmur_locations, Systolic_murmur_timing,
        ...
        for patient_id in patient_id_list:
            for pos in positoin:
                ...
                if os.path.exists(tsv_path):
                    state_div(tsv_path, wav_path, dir_path + "\\",
                              patient_id + pos, Systolic_murmur,
                              Diastolic_murmur, Systolic_state,
                              Diastolic_state)


# preprocessed PCGs were segmented into four heart sound states
def state_div(tsvname, wavname, state_path, index, Systolic_murmur,
              Diastolic_murmur, Systolic_state, Diastolic_state):
    ...
    for i in range(index_file.shape[0] - 3):
        if index_file[i][2] == '2' and index_file[i + 2][2] == '4':
            ...
            soundfile.write(
                state_path + "{}_{}_{}_{}_{}.wav".format(
                    index, 'Systolic', num, Systolic_murmur, Systolic_state),
                buff2, fs)
            soundfile.write(
                state_path +
                "{}_{}_{}_{}_{}.wav".format(index, 'Diastolic', num,
                                            Diastolic_murmur, Diastolic_state),
                buff4, fs)
...
pd.DataFrame(Murmur_locations).to_csv(Murmur_locations_path,
                                      index=False,
                                      header=False)
pd.DataFrame(Systolic_murmur_timing).to_csv(Systolic_murmur_timing_path,
                                            index=False,
                                            header=False)
pd.DataFrame(Diastolic_murmur_timing).to_csv(Diastolic_murmur_timing_path,
                                             index=False,
                                             header=False)
