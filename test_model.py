...
def run_model(model_folder, data_folder, output_folder):
    ...
    for patient_folder in os.listdir(data_folder):
        ...
        for dir in os.listdir(patient_folder):
            # 读取测试集标签和特征
            features_t, label_t, names_t, index_t, data_id_t, feat_t = get_wav_data(dir, num=0)
            ...
            with torch.no_grad():
                for batch_idx,(data_t, target_t, index_t, feat_t)  in enumerate(test_loader):
                    data_t, target_t, index_t, feat_t =data_t.to(device), target_t.to(device), index_t.to(device), feat_t.to(device) 
                    optimizer.zero_grad()
